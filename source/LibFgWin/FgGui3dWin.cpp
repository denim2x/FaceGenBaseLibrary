    ...
    struct      D3d
    {
      ...
      D3d(HWND hwnd)
      {
        ...
        {   // Pixel shader
          ...
          {
            auto data = LoadHSLS(IDR_PS);
            ID3DBlob* pErrorBlob = nullptr;
            ID3DBlob* ptr;
            //hr = D3DCompile(data.c_str(), data.size(), nullptr, nullptr, nullptr, "PS", "ps_4_0", dwShaderFlags, 0, &ptr, &pErrorBlob);
            hr = D3DCompile(data.c_str(), data.size(), nullptr, nullptr, nullptr, "FragmentCountPS", "ps_5_0", dwShaderFlags, 0, &ptr, &pErrorBlob);
            if (pErrorBlob) {       // Compilation error is not the same as function error
              const char* errPtr = static_cast<const char*>(pErrorBlob->GetBufferPointer());
              size_t          errSz = pErrorBlob->GetBufferSize();
              string          errStr(errPtr, errSz);
